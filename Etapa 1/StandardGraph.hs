...
removeNode node graph = (S.delete node (nodes graph), S.filter f (edges graph))
...
splitNode old news graph = (S.union ns (S.fromList news), S.union es (S.fromList newes))
    where
        -- Graful din care am scos vechiul nod
        (ns, es) = removeNode old graph
...
mergeNodes prop node graph = (if filtered == (nodes graph)
                            then (nodes graph)
